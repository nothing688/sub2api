...
const (
	...
	// Free Grok OAuth (~1M tokens/day) trips short RPM-style 429s constantly.
	// Multi-minute fallbacks waste the rest of the pool; prefer short cooldowns
	// and trust upstream retry-after / window reset when present.
	grokRateLimitFallbackCooldown     = 45 * time.Second
	grokRateLimitFreeFallbackCooldown = 30 * time.Second
	grokRateLimitFreeMaxCooldown      = 2 * time.Minute
...
func grokRateLimitResetAtForAccount(account *Account, snapshot *xai.QuotaSnapshot, now time.Time) (time.Time, bool) {
	...
	// RPM 429s must not park a free account for 10m/30m/1h while siblings sit idle.
	if isKnownGrokFreeAccount(account) {
		if !resetAt.After(now) {
			resetAt = now.Add(grokRateLimitFreeFallbackCooldown)
		}
		maxFree := now.Add(grokRateLimitFreeMaxCooldown)
		exhaustedFar := false
		if snapshot != nil {
			for _, window := range []*xai.QuotaWindow{snapshot.Requests, snapshot.Tokens} {
				if window == nil || window.Remaining == nil || *window.Remaining > 0 {
		...
		if !exhaustedFar && resetAt.After(maxFree) {
			resetAt = maxFree
		}
		return resetAt, true
	}
...
func normalizeGrokRateLimitResetAt(account *Account, resetAt, now time.Time) time.Time {
	if !resetAt.After(now) {
		...
		if isKnownGrokFreeAccount(account) {
			fallback = grokRateLimitFreeFallbackCooldown
		}
